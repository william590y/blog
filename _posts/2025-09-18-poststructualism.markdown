...
Setting aside a potential [no-true scotsman fallacy](https://en.wikipedia.org/wiki/No_true_Scotsman), I think a point that is commonly unadressed here in what sense a "world" is necessary for cognition. If one is arguing that language models currently struggle to understand 3D space and physics, [I couldn't agree more](https://adamkarvonen.github.io/machine_learning/2025/04/13/llm-manufacturing-eval.html). I think one of the best litmus tests for whether an LLM can solve a problem is whether drawing a diagram or geometric intuition is necessary. LLMs seem to struggle with 3D space in the same way we struggle with 4D space.
